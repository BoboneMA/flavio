...
class TransversityAmpsDeltaC_77p_polynomial(HelicityAmpsDeltaC):
    r"""Transversity amps from an effective shift in the Wilson coefficients
    $C_7$ and $C_7'$ as a polynomial in $q^2$. Computed from the shifts in the helicity 
    amplitudes which are then translated into the transversity amplitudes."""
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def __call__(self):
        ...
        ha['pl', 'V'] = self.ha_deltaC(deltaC7p_pl, '7p')['pl', 'V']
        ha['mi', 'V'] = self.ha_deltaC(deltaC7_mi, '7')['mi', 'V']
        # the others are set to zero to avoid KeyErrors
        ha['0', 'A'] = 0
        ha['pl', 'A'] = 0
        ha['mi', 'A'] = 0
        return angular.helicity_to_transversity(ha)

class TransversityAmpsDeltaC_9_shift(HelicityAmpsDeltaC):
    r"""Transversity amps from an effective $q^2$-independent shift in the Wilson
    coefficients."""
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
def fct_deltaC7C7p_polynomial_transversity(B, V):
    def fct(wc_obj, par_dict, q2, cp_conjugate):
        par = par_dict.copy()
        return TransversityAmpsDeltaC_77p_polynomial(B, V, par, q2)()
    return fct


def fct_deltaC9_constant_transversity(B, V):
    def fct(wc_obj, par_dict, q2, cp_conjugate):
        par = par_dict.copy()
        return TransversityAmpsDeltaC_9_shift(B, V, par, q2)()
    return fct

# AuxiliaryQuantity & Implementation: subleading effects at LOW q^2
for had in [('Bs','phi'), ]:  # for now only Bs->phill
    process = had[0] + '->' + had[1] + 'll' # e.g. Bs->phimumu
    quantity = process + ' subleading effects at low q2 (transversity)'
    a = AuxiliaryQuantity(name=quantity, arguments=['q2', 'cp_conjugate'])
    a.description = ('Contribution to ' + process + ' transversity amplitudes from'
                    ' subleading hadronic effects (i.e. all effects not included'
                    r'elsewhere) at $q^2$ below the charmonium resonances')

    # Implementation: C7-C7'-polynomial
    iname = process + ' deltaC7, 7p polynomial (transversity)'
    i = Implementation(name=iname, quantity=quantity,
                       function=fct_deltaC7C7p_polynomial_transversity(B=had[0], V=had[1]))
    i.set_description(r"Effective shift in the Wilson coefficient $C_7(\mu_b)$"
                      r" and $C_7'(\mu_b)$ translated to the transversity amplitudes"
                      r" as a first-order polynomial in $q^2$.")

# AuxiliaryQuantity & Implementation: subleading effects at HIGH q^2
for had in [('Bs','phi'), ]:  # for now only Bs->phill
    process = had[0] + '->' + had[1] + 'll' # e.g. Bs->phimumu
    quantity = process + ' subleading effects at high q2 (transversity)'
    a = AuxiliaryQuantity(name=quantity, arguments=['q2', 'cp_conjugate'])
    a.description = ('Contribution to ' + process + ' transversity amplitudes from'
                    ' subleading hadronic effects (i.e. all effects not included'
                    r'elsewhere) at $q^2$ above the charmonium resonances')
    ...
    # Implementation: C9 constant shift
    iname = process + ' deltaC9 shift (transversity)'
    i = Implementation(name=iname, quantity=quantity,
                       function=fct_deltaC9_constant_transversity(B=had[0], V=had[1]))
    i.set_description(r"Effective constant shift in the Wilson coefficient $C_9(\mu_b)$.")
