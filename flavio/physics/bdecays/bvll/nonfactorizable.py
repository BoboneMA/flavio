...
def tr_qcdf_function(B, V):
    ...
    def function(wc_obj, par_dict, q2, cp_conjugate):
        ...
        if cp_conjugate:
            par = conjugate_par(par)
            wc = conjugate_wc(wc)
        return flavio.physics.bdecays.bvll.qcdf.transversity_amps_qcdf(q2, wc, par, B, V)
    return function
...
# ... and the same for the interpolated version (see qcdf_interpolate.py)
def tr_qcdf_interpolate_function(B, V, contribution='all'):
    scale = config['renormalization scale']['bvll']
    def function(wc_obj, par_dict, q2, cp_conjugate):
        ha = flavio.physics.bdecays.bvll.qcdf_interpolate.helicity_amps_qcdf(q2, par_dict, B, V, cp_conjugate, contribution)
        ha[('0', 'A')] = np.zeros_like(ha['0', 'V'])
        ha[('mi', 'A')] = np.zeros_like(ha['0', 'V'])
        ha[('pl', 'A')] = np.zeros_like(ha['0', 'V'])
...
for had in [('Bs','phi'), ]:  # for now only for Bs->phimumu
    process = had[0] + '->' + had[1] + 'll'  # e.g. Bs->phimumu
    quantity = process + ' spectator scattering (transversity)'
    a = AuxiliaryQuantity(name=quantity, arguments=['q2', 'cp_conjugate'])
    a.description = ('Contribution to ' + process + ' transversity amplitudes from'
                    ' non-factorizable spectator scattering.')

    # Implementation: interpolated QCD factorization (transversity)
    iname = process + ' QCDF interpolated (transversity)'
    i = Implementation(name=iname, quantity=quantity,
                       function=tr_qcdf_interpolate_function(B=had[0], V=had[1]))
    i.set_description("Interpolated version of QCD factorization (transversity)")

    quantity = process + ' spectator scattering (transversity - no interp)'
    a = AuxiliaryQuantity(name=quantity, arguments=['q2', 'cp_conjugate'])
    a.description = ('Contribution to ' + process + ' transversity amplitudes from'
                    ' non-factorizable spectator scattering (without interpolation).')

    # Implementation: QCD factorization
    iname = process + ' QCDF (transversity)'
    i = Implementation(name=iname, quantity=quantity,
                       function=tr_qcdf_function(B=had[0], V=had[1]))
    i.set_description("QCD factorization (transversity)")